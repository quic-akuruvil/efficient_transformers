...
"""Three-way token parity test for the Gemma4-MoE disaggregated prefill/decode DMA path.
pytest -m "on_qaic and multimodal" tests/transformers/disaggregated/test_gemma4_moe_disagg_kv_share_w_hf_ort_qaic.py
"""
...
def _resolve_ort_dim(dim, seq_len: int) -> int:
    ...
    if dim in {"ctx_len", "context_length", "past_sequence_length", "sliding_window"}:
        return CTX_LEN
    if dim in {"vision_size"}:
        return VISION_SIZE
    if dim in {"height", "image_height"}:
        return IMAGE_SIZE[1]
    if dim in {"width", "image_width"}:
        return IMAGE_SIZE[0]
