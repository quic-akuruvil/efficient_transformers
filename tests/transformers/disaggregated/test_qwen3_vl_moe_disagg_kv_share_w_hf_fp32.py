...
NIGHTLY_TEST_CONFIG = _nightly_disagg_test_config(
    model_env="QEFF_QWEN3_VL_MOE_THREE_WAY_MODEL_NAME",
    default_model_name="Qwen/Qwen3-VL-30B-A3B-Instruct",
    compile_env_prefix="QEFF_QWEN3_VL_MOE",
    default_image_size=IMAGE_SIZE,
    default_image_url="https://picsum.photos/id/237/536/354",
    image_file_env="QEFF_QWEN3_VL_MOE_THREE_WAY_IMAGE_FILE",
)
...
def _load_three_way_parity_image() -> Image.Image:
    image_file = os.environ.get(NIGHTLY_TEST_CONFIG.image_file_env)
    try:
        if image_file:
            image = Image.open(image_file).convert("RGB")
            print(f"Loaded Qwen3-VL three-way parity image from {image_file}")
        else:
            requests = pytest.importorskip("requests")
            response = requests.get(NIGHTLY_TEST_CONFIG.image_url, stream=True, timeout=60)
            response.raise_for_status()
            image = Image.open(response.raw).convert("RGB")
            print(f"Loaded Qwen3-VL three-way parity image from {NIGHTLY_TEST_CONFIG.image_url}")
    except Exception as exc:
        pytest.skip(
            "Qwen3-VL three-way parity image could not be loaded; set "
            f"{NIGHTLY_TEST_CONFIG.image_file_env} to a cached copy. Error: {exc}"
        )
    print(f"Qwen3-VL three-way parity image size: width={image.width}, height={image.height}")
    return image
...
def _suppressed_eos_token_ids(processor: AutoProcessor) -> tuple[int, ...]:
...
def _get_next_token_ids(logits: np.ndarray, suppressed_token_ids: tuple[int, ...] = ()) -> np.ndarray:
    logits = np.asarray(logits)
    next_token_logits = logits[:, -1, :] if logits.ndim == 3 else logits
    if suppressed_token_ids:
        next_token_logits = next_token_logits.copy()
        vocab_size = next_token_logits.shape[-1]
        valid_token_ids = [token_id for token_id in suppressed_token_ids if 0 <= token_id < vocab_size]
        if valid_token_ids:
            next_token_logits[:, valid_token_ids] = -np.inf
    return next_token_logits.argmax(axis=-1).astype(np.int64)


def _print_token_array_comparison(
    label_a: str,
    tokens_a: np.ndarray,
    label_b: str,
    tokens_b: np.ndarray,
) -> None:
    tokens_a = np.asarray(tokens_a)
    tokens_b = np.asarray(tokens_b)
    limit = min(tokens_a.shape[-1], tokens_b.shape[-1])
    if limit == 0:
        print(f"{label_a} vs {label_b} comparison skipped: empty token sequence")
        return

    matching_steps = (tokens_a[:, :limit] == tokens_b[:, :limit]).all(axis=0)
    num_matched = int(matching_steps.cumprod().sum())
    print(f"{label_a} vs {label_b} matched leading tokens : {num_matched}/{limit}")
    if matching_steps.all() and tokens_a.shape[-1] == tokens_b.shape[-1]:
        return

    if num_matched < limit:
        print(
            f"{label_a} vs {label_b}: first mismatch at token index {num_matched}: "
            f"{label_a}={tokens_a[:, num_matched].tolist()} vs "
            f"{label_b}={tokens_b[:, num_matched].tolist()}"
        )
    else:
        print(
            f"{label_a} vs {label_b}: prefixes match but lengths differ: {tokens_a.shape[-1]} vs {tokens_b.shape[-1]}"
        )
...
def _patch_custom_rmsnorm_for_ort(path: Path) -> Path:
    ...
    Fixes applied per function type:
      ...
      CtxScatter*      : Cast ScatterND indices to int64; clamp int32.max sentinel
                         in position_ids to a safe, future cache slot.
            ...
            if pos_param is not None:
                new_nodes.extend(
                    [
                        ...
                        _make_const_i64_1d(
                            "_sc_ctx_dim_idx",
                            [1 if function.name in {"CtxScatter3D", "CtxScatter3DInt"} else 2],
                        ),
                        helper.make_node("Gather", ["_sc_data_shape", "_sc_ctx_dim_idx"], ["_sc_ctx_dim"]),
...
def _run_ort_generation(
    ...
    suppressed_token_ids = _suppressed_eos_token_ids(processor)
    ...
    decode_input_names = _session_input_names(decode_session)
    decode_has_image_idx = "image_idx" in decode_input_names
    print(f"[ORT] decode_inputs  : {decode_input_names}", flush=True)
    ...
    first_token = _get_next_token_ids(prefill_outputs["logits"], suppressed_token_ids)
    ...
    if decode_has_image_idx and "image_idx_output" in prefill_outputs:
    ...
    for step_idx in range(GENERATION_LEN - 1):
        ...
        tok = _get_next_token_ids(decode_outputs["logits"], suppressed_token_ids)
        print(
        ...
        if decode_has_image_idx and "image_idx_output" in decode_outputs:
...
def _run_disagg_kv_share_qaic_generation(
    ...
    generated_ids = [_get_next_token_ids(prefill_out["logits"], suppressed_token_ids)]
    ...
    if decode_has_image_idx and "image_idx_output" in prefill_out:
    ...
    for _ in range(GENERATION_LEN - 1):
        ...
        generated_ids.append(_get_next_token_ids(decode_outputs["logits"], suppressed_token_ids))
        phys_pos += 1
        mrope_pos += 1
        decode_inputs = {
            "input_ids": generated_ids[-1].reshape(BATCH_SIZE, 1),
...
def _compile_disagg_sessions(
    ...
    *,
    ...
    decode_num_devices: int = 1,
    stages: int = 2,
    prefill_enable_depth_first: bool = True,
    prefill_offload_pt_weights: bool | None = None,
):
    print(
        "Disagg compile config: "
    ...
    prefill_compile_kwargs = {
        ...
        "mxfp6_matmul": False,
        "mxint8_kv_cache": False,
        "aic_enable_depth_first": prefill_enable_depth_first,
    ...
    if prefill_offload_pt_weights is not None:
        prefill_compile_kwargs["offload_pt_weights"] = prefill_offload_pt_weights
...
def test_qwen3_vl_moe_disagg_kv_share_qaic_vs_ort_fp32(manual_cleanup):
    ...
    image = _load_three_way_parity_image()
    ...
    try:
        ...
        _print_token_array_comparison("HF fp32", hf_tokens, "ORT fp32", ort_tokens)
        ...
        vision_session, prefill_session, decode_session = _compile_disagg_sessions(
            ...
            sessions,
            compiled_onnx_paths,
            vision_num_devices=NIGHTLY_TEST_CONFIG.vision_num_devices,
            ...
            prefill_enable_depth_first=False,
            prefill_offload_pt_weights=False,
        )
