...
pytest -m "on_qaic and multimodal" tests/transformers/disaggregated/PR1150/final_tests/test_qwen3_5_disagg_kv_share_w_hf_ort_qaic.py
"""
import copy
import os
import warnings
        ...
        if function.name == "CustomRMSNorm":
            for node in function.node:
                if (node.op_type == "Cast"
                        and list(node.input) == ["weight"]
                        and list(node.output) == ["weight_0"]):
                    node = helper.make_node(
                        "CastLike", ["weight", "hidden_states"], ["weight_0"],
                ...
                if node.op_type == "Expand" and list(node.output) == ["epsilon_2"]:
                    ...
                    new_nodes.append(helper.make_node(
                        "CastLike", ["epsilon_2_pre_cast", "variance"], ["epsilon_2"],
        ...
        # --------------------------------------------------------------
            ...
            if pos_param is not None:
                new_nodes.extend([
                    ...
                    helper.make_node("Cast", ["_sc_safe_pos"], ["_sc_pos_i64"],
                                     name="Cast_sc_pos_i64", to=TensorProto.INT64),
                ])
                fn_changed = True
            ...
            for node in function.node:
                ...
                if node.op_type == "ScatterND":
                    ...
                    if not indices_in.endswith("_i64"):
                        ...
                        new_nodes.append(helper.make_node(
                            "Cast", [indices_in], [cast_out],
                            name=f"Cast_{indices_in}_i64", to=TensorProto.INT64,
                        ))
                ...
                if node.op_type == "Expand" and list(node.output) == ["ctx_idx"]:
                    ...
                    new_nodes.append(node)
                    new_nodes.append(helper.make_node(
                        "Cast", ["ctx_idx_pre_i64"], ["ctx_idx"],
                        name="Cast_ctx_idx_i64", to=TensorProto.INT64,
                    ))
                    fn_changed = True
                    continue
        ...
        # --------------------------------------------------------------
            ...
            if clamp_target is not None:
                new_nodes.extend([
                    ...
                    helper.make_node("Cast", ["_gc_safe_target"], ["_gc_target_i64"],
                                     name="Cast_gc_target_i64", to=TensorProto.INT64),
                ])
                fn_changed = True
            ...
            for node in function.node:
                ...
                if (not inserted_ctx_indices_cast
                    ...
                    new_nodes.append(helper.make_node(
                        "Cast", [ctx_indices_param], [cast_name],
                        name=f"Cast_{ctx_indices_param}_i64", to=TensorProto.INT64,
                    ))
...
def test_qwen3_5_disagg_kv_share_logit_diff_diagnostic(manual_cleanup):
    """Optional diagnostic: compares HF fp32 vs disagg-QAIC logits step-by-step.
    ...
    print(f"\nHF   tokens : {hf_tokens.tolist()}")
    print(f"QAIC tokens : {qaic_tokens.tolist()}")

    num_steps = min(hf_logits.shape[1], qaic_logits.shape[1])
    mismatches = []
    for step in range(num_steps):
        hf_tok = int(np.argmax(hf_logits[0, step]))
        qaic_tok = int(np.argmax(qaic_logits[0, step]))
        if hf_tok != qaic_tok:
            mismatches.append(step)
            _report_step_logits(step, hf_logits[:, step, :], qaic_logits[:, step, :], processor.tokenizer)

    if not mismatches:
        print("\nAll logits agree — no token mismatches detected.")
    else:
