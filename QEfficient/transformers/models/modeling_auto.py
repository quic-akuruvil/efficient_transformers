...
def _configure_vlm_moe_expert_parallel(
    ...
    *,
    ...
    if cores_per_expert <= 0:
        raise ValueError("`cores_per_expert` must be greater than 0 when configuring MoE expert parallelism.")
    if not isinstance(tree_reduce, bool):
        raise TypeError("`tree_reduce` must be a boolean.")

    # `num_devices` is the full device mesh handed to the compiler. Only a disaggregated
    # prefill session splits that mesh into `mdp_num_partitions` pipeline stages, so only
    # there is the expert-parallel core budget the tensor-sliced slice of the mesh
    # (num_devices // mdp_num_partitions devices, each contributing num_cores NSPs).
    # Decode (and any non-disaggregated) compile keeps the historical full-mesh budget.
    ...
    for module in model.modules():
        ...
        if num_pipeline_stages <= 0:
            raise ValueError("`num_pipeline_stages` must be greater than 0 for MoE expert parallelism.")
        ...
        module.num_nsp = num_parallelized_experts
        module.local_experts = num_pipeline_stages
...
class _QEffAutoModelForImageTextToTextDualQPC:
    ...
    def export(
        ...
        **kwargs :
        ...
        export_seq_len = min(prefill_seq_len, constants.ONNX_EXPORT_EXAMPLE_SEQ_LEN) if prefill_seq_len else None
